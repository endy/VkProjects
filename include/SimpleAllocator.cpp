...
void VKAPI_PTR SimpleInternalAllocNotify(
    ...
    VkSystemAllocationScope                     allocationScope)
{
    SIMPLE_PRINT("internal alloc notify\n");
}
...
void VKAPI_PTR  SimpleInternalFreeNotify(
    ...
    VkSystemAllocationScope                     allocationScope)
{
    SIMPLE_PRINT("internal free notify");
}

const VkAllocationCallbacks* GetSimpleAllocator()
{
    static char* UserData = "Muh user datah\n";

    static const VkAllocationCallbacks SimpleAllocator =
    {
        &UserData[0],
        SimpleAlloc,
        SimpleRealloc,
